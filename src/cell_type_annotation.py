...
# ---------------------------------------------------------------------------
# Cluster-label lookup derived from dotplot analysis (fig4_dotplot.pdf).
# Mapping: Leiden cluster ID (as string) → descriptive cell type label.
# Generated by cross-referencing rank_genes_groups dotplot markers with
# known MBH cell biology (AGED/ADULT mouse brain, 16 Leiden clusters at
# resolution=0.5 on 92 689 MBH cells).
# ---------------------------------------------------------------------------
MBH_CLUSTER_LABELS: dict[str, str] = {
    # --- Inhibitory neurons ---
    "0":  "GABAergic neuron (Otp+/Gal+)",       # Gad2/Slc32a1+, Otp+ hypothalamic TF
    "1":  "GABAergic neuron",                    # Pan-GABAergic, Pdyn+
    "3":  "GABAergic neuron (Necab1+)",          # Necab1 = Sncg-type interneuron marker
    "7":  "GABAergic neuron (Brs3+)",            # Brs3/Gast+ MBH subtype (custom genes)
    "10": "GABAergic neuron (Penk+)",            # Enkephalinergic
    "11": "GABAergic neuron (Pvalb+/Tac1+)",     # Pvalb = panel 'Pvalb interneuron'
    # --- Excitatory neurons ---
    "2":  "Glutamatergic neuron (Adcyap1+)",     # Slc17a6/VGluT2+ (panel='Cajal-Retzius' but is VGluT2 in MBH)
    "4":  "Glutamatergic neuron (Otp+)",         # Slc17a6/VGluT2+, Otp+
    "6":  "Glutamatergic neuron (Crh+)",         # Slc17a6+, Crh+ (panel='Vip' but is PVN/DMH in MBH)
    # --- Specialised peptidergic ---
    "5":  "Hcrt/Orexin neuron",                  # Hcrt definitive; Cartpt+ (both custom genes)
    # --- Glia ---
    "8":  "Astrocyte",                           # Aqp4/Ntsr2/Slc39a12/Acsbg1 = panel 'Astrocytes'
    "9":  "Oligodendrocyte",                     # Opalin/Sox10/Gjc3/Zfp536 = panel 'Oligodendrocytes'
    "13": "OPC",                                 # Gpr17/Sema3d = panel 'Oligodendrocytes'; Pdgfra = OPC in MBH
    "15": "Oligodendrocyte (maturing)",          # Sox10+; mixed Gfap signal — flag for sub-clustering
    # --- Microglia ---
    "12": "Microglia (Spp1-hi / DAM)",           # Trem2/Siglech/Laptm5/Cd53 = panel 'Microglia/PVM'; Spp1 = DAM
    "14": "Microglia (Siglech-hi)",              # Siglech-hi subtype; panel 'Microglia/PVM'
}
...
def assign_cluster_labels(
    ...
    """
    Assign cell type labels to clusters using a pre-defined lookup table.

    This is the recommended method after visually inspecting the dotplot
    (fig4_dotplot.pdf) and cross-referencing with known MBH cell biology.

    Parameters
    ----------
    adata:
        AnnData with Leiden cluster labels in .obs[cluster_key].
    cluster_key:
        Column in .obs containing cluster IDs (default: "leiden").
    label_map:
        Dict mapping cluster ID string -> label string.
        Defaults to MBH_CLUSTER_LABELS derived from the AGED/ADULT dotplot.
    new_col:
        Name of the new .obs column to create (default: "cell_type").
    fallback:
        Label for any cluster not found in label_map.

    Returns
    -------
    AnnData with .obs[new_col] populated and cast to Categorical.
    """
    if label_map is None:
        label_map = MBH_CLUSTER_LABELS
    ...
    n_labelled = (adata.obs[new_col] != fallback).sum()
    logger.info(
        "Cluster labels assigned: %d / %d cells labelled (%d clusters mapped)",
        n_labelled, adata.n_obs,
        adata.obs[cluster_key].nunique(),
    )
    # Log the mapping
    summary = (
        adata.obs.groupby(cluster_key, observed=True)[new_col]
        .first()
        .reset_index()
    )
    for _, row in summary.iterrows():
        logger.info("  Cluster %-4s -> %s", row[cluster_key], row[new_col])
    return adata
